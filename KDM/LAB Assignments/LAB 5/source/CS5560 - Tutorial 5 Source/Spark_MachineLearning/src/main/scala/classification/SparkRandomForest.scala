...
//  private def run(params: Params) {
    ...
    val conf = new SparkConf().setAppName(s"RFExample with" ).setMaster("local[*]").set("spark.driver.memory", "4g").set("spark.executor.memory", "4g")
    val sc = new SparkContext(conf)


    val inputPath="C:\\Users\\syedm\\Desktop\\Yahoo-Questions-traindata.csv"
    val test="E:\\UMKC\\Sum_May\\KDM\\Week 5\\Yahoo-Question-testdata.csv"

    Logger.getRootLogger.setLevel(Level.WARN)

    val topic_output = new PrintStream("C:\\Users\\syedm\\Desktop\\CS5560 - Tutorial 5 Source\\Spark_MachineLearning\\data\\RF_Results.txt")
    // Load documents, and prepare them for RF.
    val preprocessStart = System.nanoTime()
    val (inputVector, corpusData, vocabArray) = preprocess(sc, inputPath)
...
/*    var hm = new HashMap[String, Int]()
  ...
  private def preprocess(sc: SparkContext,paths: String): (RDD[Vector], RDD[(String,String)], Long) = {
    ...
    //Reading Stop Words
    val stopWords=sc.textFile("C:\\Users\\syedm\\Desktop\\CS5560 - Tutorial 5 Source\\Spark_MachineLearning\\data\\stopwords.txt").collect()
